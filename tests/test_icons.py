...
def test_icon_sequence_has_length():
    config = ImageConfig()
    icons = Icons(None, config, 1, ["M", "T", "W"], date(2023, 6, 5), date(2023, 6, 7), MoonPhase.FULL)
    assert len(icons) == 4
...
def test_should_convert_icon_collection_to_string():
    icons = Icons(None, ImageConfig(), 10, ["S", "M", "T", "W"], date(2023, 6, 4), date(2023, 6, 7), MoonPhase.FULL)
