...
def test_icon_sequence_has_length():
    config = ImageConfig()
    icons = Icons(config, 1, date(2023, 6, 5), date(2023, 6, 8), MoonPhase.FULL)
    assert len(icons) == 4

def test_icon_sequence_can_be_iterated_over():
    config = ImageConfig()
    icons = Icons(config, 1, date(2023, 6, 5), date(2023, 6, 7), MoonPhase.FULL)
    for icon in icons:
        pass
...
def test_should_convert_icon_collection_to_string():
    icons = Icons(ImageConfig(), 10, date(2023, 6, 4), date(2023, 6, 8), MoonPhase.FULL)
...
def test_letters(start, end, seq):
    icons = Icons(ImageConfig(), 0, start, end, MoonPhase.FULL)
    assert str(icons) == seq

def test_should_raise_error_when_index_too_large():
    icons = Icons(ImageConfig(), 1, date(2023, 6, 5), date(2023, 6, 7), MoonPhase.FULL)
